...
#### 核心拦截功能（参考 augment-account-manager）
- ✅ **Session ID 替换** - 所有请求中的 Session ID 自动替换为伪造 ID
- ✅ **Conversation ID 映射** - 随机替换 conversation_id（仅服务器端）
- ✅ **Blobs 清理** - 清空 blobs 数组（仅服务器端）
- ✅ **Machine ID 伪造** - 拦截 vscode.env.machineId 并返回伪造 ID
- ✅ **Feature Vector 替换** - 递归替换所有 64 字符 hex Feature Vector
- ✅ **本地上下文保留** - LevelDB 存储原始数据，VSCode 重启后自动恢复
...
#### 拦截的端点
- ✅ chat-stream - 聊天流式响应（清空 blobs + 替换 conversation_id）
- ✅ record-request-events - 请求事件记录（递归替换 conversation_id）
- ✅ report-feature-vector - Feature Vector 上报（替换 Vector）

#### 白名单保护（参考 augment-account-manager）
- ✅ **必要端点白名单** - 保护核心功能端点（chat-stream, codebase-retrieval 等）
- ✅ **代码索引保护** - 保护代码索引相关功能（batch-upload, symbol-index 等）
- ✅ **智能分类** - 区分遥测数据和功能数据
...
1. **上下文保留机制**:
   - ✅ 优点: 保留完整聊天上下文，AI 能记住之前的对话
   - ✅ 优点: VSCode 重启后聊天记录不丢失
   - ⚠️ 注意: 会保留所有请求体数据（包括文件内容）
   - 💡 建议: 不要在聊天中包含敏感信息

2. **Conversation ID 保留**:
   - ✅ 优点: 维持对话连续性，上下文不会断裂
   - ⚠️ 注意: 与 release1 不同，不替换对话 ID
   - 💡 说明: 这是保留上下文的关键机制
