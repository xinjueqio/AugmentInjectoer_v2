...
class AugmentTokenLoginEnhanced {
  ...
  async restoreTokenOnStartup() {
    // 强制输出到控制台,确保可见
    console.log('[TokenLogin] ========== restoreTokenOnStartup() called ==========');

    try {
      console.log('[TokenLogin] Attempting to restore token and session on startup...');
      ...
      if (tokenData.success && tokenData.accessToken) {
        console.log('[TokenLogin] ✅ Found stored token');
        console.log('[TokenLogin] Tenant URL: ' + tokenData.tenantURL);
        ...
        if (restoredSessionId) {
          console.log('[TokenLogin] ✅ Session ID restored from storage: ' + restoredSessionId);
          ...
          console.log('[TokenLogin] No saved session ID, generating new one...');
          this.logger.info('No saved session ID, generating new one...');
          await this.updateInterceptorSessionId();
        }

        console.log('[TokenLogin] ✅ Token and session restored successfully');
        console.log('[TokenLogin] You are logged in and ready to use Augment');
        this.logger.info('✅ Token and session restored successfully');
        this.logger.info('You are logged in and ready to use Augment');
      } else {
        console.log('[TokenLogin] ℹ️ No stored token found - please login first');
        this.logger.info('ℹ️ No stored token found - please login first');
      }
    } catch (error) {
      console.error('[TokenLogin] ❌ Failed to restore token on startup:', error);
      this.logger.error('Failed to restore token on startup:', error);
      // 不抛出错误,允许扩展继续初始化
    }

    console.log('[TokenLogin] ========== restoreTokenOnStartup() completed ==========');
  }
